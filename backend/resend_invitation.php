...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($member['status'] !== 'pending') {
            ...
            echo json_encode(['success' => false, 'message' => 'Only pending invitations can be resent']);
        ...
        $expires = new MongoDB\BSON\UTCDateTime((time() + (48 * 3600)) * 1000);
        ...
        $inviteeName = $member['display_name'] ?? 'there';
        ...
        $subject = sprintf($lang['email_invite_subject_resent'], $product_name);



        // Outlook-ready table-based template (Resent)
                        ...
                        <table border='0' cellpadding='0' cellspacing='0' width='100%' style='max-width: 600px;'>
                            ...
                            <tr>
                                ...
                                <td align='left' style='padding-bottom: 20px; color: #4b5563; font-size: 16px; line-height: 24px;'>
                                    ...
                                    <p style='margin: 0;'>" . sprintf($lang['email_invite_text_resent'], "<strong>{$product_name}</strong>", "<strong>{$position}</strong>") . "</p>
        ...
        $emailResult = sendResendEmail($member['email'], $subject, $html);
        ...
        if ($emailResult === true || (is_array($emailResult) && ($emailResult['success'] ?? false))) {
            ...
            echo json_encode(['success' => false, 'message' => 'Failed to send email. ' . ($errorDetail ?: 'Check Resend configuration.')]);
